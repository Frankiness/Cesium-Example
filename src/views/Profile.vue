<template>
  <div id="cesium-container">
    <div id="chart"></div>
    <ButtonGroup
      ><button @click.stop="sampleTiles">采样3dtiles</button>
      <button @click.stop="sampleTerrain">采样地形</button>
    </ButtonGroup>
  </div>
</template>
...
function loadTiles() {
  const tileset = new Cesium.Cesium3DTileset({
    url: `http://localhost:9003/model/QSf5aZto/tileset.json`,
...
function sampleTerrain() {
  Profile(viewer, Cesium, "terrain");
}
function sampleTiles() {
  Profile(viewer, Cesium, "tiles");
}
